...
# aaa = intersec(matrix_data,new_matrix_data)
# print(aaa)
...
# result_structure = CifParser(cif_file_path).parse_structures()[0]
...
# print("bond Center is",center_pos[the_bond_index][the_bond_subindex])

# print(bonds_dict)
# print("center_pos is ",center_pos[1][1])
...
#                         [0, 0, -1]])
# product = A @ dr
# aMat, aSym = basic_sym_matrix(pOp, product, 1e-5)
# get_symm_anti_mat(aMat,aSym)

# subidx1 = np.array([[-0.093 ,   3.773  ,   1.049],
#               [3.773  ,   4.267  ,   1.817],
#               [1.049  ,   1.817  ,   4] ])

# idx3sub1 =  np.array([[1, 0.317,0.759],
#                      [0.317,1.366,1.314],
#                      [0.759, 1.314,  1]])

# bb = np.dot(lattice_matrix,np.dot(symmetry_dataset['rotations'][2],np.linalg.inv(lattice_matrix)))
# # print(symmetry_dataset['rotations'][2])
# aaaa = np.dot(bb,np.dot(idx3sub1,np.linalg.inv(bb)))
# aaaa = np.dot(symmetry_dataset['rotations'][4].T,np.dot(aa,np.linalg.inv(symmetry_dataset['rotations'][4].T)))
# print(symmetry_dataset['rotations'])
# print(aaaa)
# # print(structure)
# print(symmetry_dataset)

# cc =np.array([[ 3.40031121e+00, -1.79676972e-04,  0.00000000e+00],
#  [-1.79676972e-04,  1.99688790e-01,  0.00000000e+00],
#  [ 0.00000000e+00,  0.00000000e+00,  3.00000000e+00]])

# print(np.linalg.eig(cc))

# orth = np.array([[ 0.30322,  -0.3409,  0.0148879],
#                  [  0.3282,  -0.0277,  -0.316118],
#                  [ 0.07979,  0.07979,  0.07979]])
# cri3 =  np.array([[ -2.29,  0.3 ,  0.17],
#                  [  0.3,  -1.93,   0.29],
#                  [  0.17,  0.29,  -2.23]])
# alpha_beta_gamma = np.dot(orth,np.dot(cri3,np.linalg.inv(orth)))
# print(alpha_beta_gamma)
# print(np.linalg.eig(cccc))
# cri3_2 =  np.array([[ -2.29, 0 ,  0.17],
#                  [  0,  -1.93,   0],
#                  [  0.17,  0,  -2.23]])
# abc = np.dot(lattice_matrix,np.dot(symmetry_dataset['rotations'][2],np.linalg.inv(lattice_matrix)))
# aaaa = np.dot(bb,np.dot(cri3_2,np.linalg.inv(bb)))
# print(lattice_matrix)
# from pymatgen import Structure

# 假设 result_structure 是从 CIF 文件加载的结构
# radius =3  # 设定一个搜索半径，根据你的系统可能需要调整
...
# expand_matrix = np.dot(result_structure.lattice.matrix , scaling_matrix)
# 创建一个空的 Structure 对象来构建超胞
# supercell_structure = Structure(expand_matrix, [], [])

# 对每个原子寻找邻居并将邻居加入到超胞结构中
# for site in result_structure:
#     neighbors = result_structure.get_neighbors(site, r=radius)
#     for neighbor in neighbors:
#         image_vector = np.array(neighbor.image)
#         shift = np.dot(image_vector, result_structure.lattice.matrix)
#         shifted_coords = neighbor.coords
#         supercell_structure.append(neighbor.species_string, shifted_coords,coords_are_cartesian=True)
# 确保超胞中的原子位置是唯一的（去除重复）
# supercell_structure.merge_sites()
# 打印超胞结构
# print("超胞结构包含的原子数：", len(supercell_structure))
# # print(supercell_structure)
# cif_writer = CifWriter(supercell_structure)
# cif_writer.write_file('output.cif')
# print(super_cell_scaling)
...
# print(result_structure.lattice.matrix)
print(aMat.shape[2])
...
# def get_magmom_tags(start_atoms, end_atoms, all_combinations, total_atom_number):
#     magnetic_directions = {
#         0: "3 0 0",
#         1: "0 3 0",
#         2: "0 0 3"
...
#         magmom_array = ["0 0 0" for _ in range(total_atom_number)]  # Default direction
...
# magmom_tags = get_magmom_tags(bond_start_idx_in_sc, bond_end_idx_in_sc, all_combination, supercell.num_sites)
# # for idx, tag in enumerate(magmom_tags, 1):
# #     print(f"MAGMOM Variant {idx}: {tag}")
...
# def generate_magmom_files(base_dir, start_atoms, end_atoms, all_combinations, total_atom_number, magnetic_directions, template_path,source_dir):
...
#             magmom_array = ["0 0 1" for _ in range(total_atom_number)]  # 默认方向
...
#                 # modified_content2 = [line if not line.strip().startswith('M_CONSTR') else f"M_CONSTR = {magmom_string}\n" for line in template_content] 
#                 with open(os.path.join(variant_dir, "INCAR"), 'w') as f:
#                     f.writelines(modified_content)
#                     # f.writelines(modified_content2)
#                 copy_support_files(source_dir, variant_dir, support_files)
# base_dir = r"C:\Users\wangh\OneDrive\Desktop\Codes\SymmMagHam\pyspinM\test2" 
# template_path = r"C:\Users\wangh\OneDrive\Desktop\Codes\SymmMagHam\pyspinM\test2\INCAR"  # 模板文件路径
# source_dir = r"C:\Users\wangh\OneDrive\Desktop\Codes\SymmMagHam\pyspinM\test2" 
...
#     2: "0 0 3"
# }
# # generate_magmom_files(base_dir, start_atoms, end_atoms, all_combinations, total_atoms, magnetic_directions)
# generate_magmom_files(base_dir,bond_start_idx_in_sc, bond_end_idx_in_sc, all_combination, supercell.num_sites,magnetic_directions,template_path,source_dir)


# # print(symmetry_dataset)
# a1 = np.array([-0.5,0.,0.33456])
# a2 = np.array([0., 0., 0.83456])
...
# Printing magmom_tags for demonstration
# for idx, tag in enumerate(magmom_tags, 1):
#     print(f"MAGMOM Variant {idx}: {tag}")
